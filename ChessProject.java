...
public class ChessProject extends JFrame implements MouseListener, MouseMotionListener {
   ...
   * here. Either the Black Pawn is in
   * its starting position in which case it can move either one or two squares or
   * it has already moved and the it can only
   * one square down the board. The Pawn can also take an opponent piece in a
   * diagonal movement. and if it makes it to the
   * bottom of the board it turns into a Queen (this should be handled where the
   * move is actually being made and not in this
   * method).
   */
  private Stack getWhitePawnSquares(int x, int y, String piece) {
    ...
    if (piece.contains("White")) {
        ...
        if (!(tmpy > 7)) {
          if (y == 1) {
            ...
            validM = new Move(startingSquare, tmp,
                setScore(((tmp.getXC() * 75) + 20), ((tmp.getYC() * 75) + 20), x, y));
      ...
      for (int i = 1; i < 2; i++) {
        ...
        if (!(tmpy > 7)) {
          Square tmp = new Square(tmpx, tmpy, piece);
          validM2 = new Move(startingSquare, tmp, setScore(((tmp.getXC() * 75) + 20), ((tmp.getYC() * 75) + 20), x, y));
      ...
      for (int i = 1; i < 2; i++) {
        ...
        if (!(tmpx > 7 || tmpx < 0 || tmpy > 7 || tmpy < 0)) {
          Square tmp = new Square(tmpx, tmpy, piece);
          validM3 = new Move(startingSquare, tmp, setScore(((tmp.getXC() * 75) + 20), ((tmp.getYC() * 75) + 20), x, y));
      ...
      for (int i = 1; i < 2; i++) {
        ...
        if (!(tmpx > 7 || tmpx < 0 || tmpy > 7 || tmpy < 0)) {
          Square tmp = new Square(tmpx, tmpy, piece);
          validM4 = new Move(startingSquare, tmp, setScore(((tmp.getXC() * 75) + 20), ((tmp.getYC() * 75) + 20), x, y));
   ...
   * Square.
   * The method should return true if there is no King in any of the squares
   * surrounding
   * the square that was submitted to the method. The method checks the grid
   * below:
   * 
   * 
   ...
   * returns a stack of all the possible
   * valid moves that the WhiteKing can move to.
   * 
   * So lets consider how the King can move. The King can essentially move in any
   * direction as long as there is not another
   * king in an adajacent square to were the king lands. Additionally, the King
   * can only move one square at a time.
   * 
   * To support this method we will also create a helper method called
   * checkSurroundingSquares(Square s){, see above
   * that returns a Boolean value to let us know if a supplied square will be
   * adajacent to another square with a
   * BlackKing present. Essentially if we consider that the board is a set of
   * squares with coordinates for each square (x, y), this
   * allows us to identify the possible squares that we should be investigating,
   * see below;
   * 
   ...
   * check eight other potential squares.
   * Remember we only need to check squares and contsruct moves if the movement
   * (Piece on a Square --> a new Square) is
   * going to be placing the piece back on the board, if we are not taking our own
   * piece and if the resulting landing square
  ...
  private Stack getKingSquares(int x, int y, String piece) {
    ...
    if (piece.contains("White")) {
      ...
      if (!((tmpx1 > 7))) {
         ...
         * increases.
         * If we consider x increasing, we need to make sure that we don't fall off the
         * board, so we use
         * a condition here to check that the new value of x (tmpx1) is not greater than
         * 7.
         * 
         * From the grid above we can see in this column that there are three possible
         * squares for us to check in
         * this column:
         ...
         * possibilities.
         * As the unchanged y value is already a location on the board we don't need to
         * check the location and can simply
         ...
         * condition below:
         * - firstly we create a new Move, which takes the starting square and the
         * landing square that we have just checked with
         * checkSurroundingSquares.
         * - Next we need to figure out if there is a piece present on the square and if
         * so make sure
         * that the piece is an opponents piece.
         * - Once we make sure that we are either moving to an empty square or we are
         * taking our opponents piece we can push this
         ...
         * for the other columns as identified above
         * in the grid.
         */
        ...
        if (checkSurroundingSquares(tmp)) {
          validM = new Move(startingSquare, tmp, setScore(((tmp.getXC() * 75) + 20), ((tmp.getYC() * 75) + 20), x, y));
          if (!piecePresent(((tmp.getXC() * 75) + 20), (((tmp.getYC() * 75) + 20)))) {
            moves.push(validM);
          } else {
            if (checkWhiteOponent(((tmp.getXC() * 75) + 20), (((tmp.getYC() * 75) + 20)))) {
        ...
        if (!(tmpy1 > 7)) {
          if (checkSurroundingSquares(tmp1)) {
            validM2 = new Move(startingSquare, tmp1,
                setScore(((tmp1.getXC() * 75) + 20), ((tmp1.getYC() * 75) + 20), x, y));
        ...
        if (!(tmpy2 < 0)) {
          if (checkSurroundingSquares(tmp2)) {
            validM3 = new Move(startingSquare, tmp2,
                setScore(((tmp2.getXC() * 75) + 20), ((tmp2.getYC() * 75) + 20), x, y));
      ...
      if (!((tmpx2 < 0))) {
        ...
        if (checkSurroundingSquares(tmp3)) {
          validM = new Move(startingSquare, tmp3,
              setScore(((tmp3.getXC() * 75) + 20), ((tmp3.getYC() * 75) + 20), x, y));
        ...
        if (!(tmpy1 > 7)) {
          if (checkSurroundingSquares(tmp4)) {
            validM2 = new Move(startingSquare, tmp4,
                setScore(((tmp4.getXC() * 75) + 20), ((tmp4.getYC() * 75) + 20), x, y));
        ...
        if (!(tmpy2 < 0)) {
          if (checkSurroundingSquares(tmp5)) {
            validM3 = new Move(startingSquare, tmp5,
                setScore(((tmp5.getXC() * 75) + 20), ((tmp5.getYC() * 75) + 20), x, y));
      ...
      if (!(tmpy1 > 7)) {
        if (checkSurroundingSquares(tmp7)) {
          validM2 = new Move(startingSquare, tmp7,
              setScore(((tmp7.getXC() * 75) + 20), ((tmp7.getYC() * 75) + 20), x, y));
      ...
      if (!(tmpy2 < 0)) {
        if (checkSurroundingSquares(tmp8)) {
          validM3 = new Move(startingSquare, tmp8,
              setScore(((tmp8.getXC() * 75) + 20), ((tmp8.getYC() * 75) + 20), x, y));
   ...
   * move in an x direction or
   * in a y direction as long as there is nothing in the way and it can take its
   * opponents piece but not its
   * own piece. As seen in the below grid the Rook can either move in a horizontal
   * direction (x changing value)
  ...
  private Stack getRookMoves(int x, int y, String piece) {
     ...
     * incriment up to the maximun number of squares.
     * On each iteration of the first loop we are adding the value of i to the
     * current x coordinate.
     * We make sure that the new potential square is going to be on the board and if
     * it is we create a new square and a new potential
     * move (originating square, new square).If there are no pieces present on the
     * potential square we simply add it to the Stack
     * of potential moves.
     * If there is a piece on the square we need to check if its an opponent piece.
     * If it is an opponent piece its a valid move, but we
     * must break out of the loop using the Java break keyword as we can't jump over
     * the piece and search for squares. If its not
   ...
   * below grid, the Bishop
   * can move in a diagonal moement. There are essentially four different
   * directions from a single
   * square that the Bishop can move along. The Bishop can move any distance along
   * this diagonal
   * as long as there is nothing in the way. The Bishop can also take an opponent
   * piece but cannot take its
   * own piece.
   * 
   * 
   * _|_____________|___________|_________|___________|___________|_
  ...
  private Stack getBishopMoves(int x, int y, String piece) {
     ...
     * own piece
     * it cannot jump over its own piece. We need to use four different loops to go
     * through the possible movements
     * to identify possible squares to move to. The temporary squares, i.e. the
     * values of x and y must change by the
     ...
     * new move (from the original square to the new
     * square). We then check if there is a piece present on the new square:
     * - if not we add the move as a possible new move
     * - if there is a piece we make sure that we can capture our opponents piece
     * and we cannot take our own piece
      ...
      for (int k = 1; k < 8; k++) {
        ...
        if (!(tmpk > 7 || tmpk < 0 || tmpy2 > 7 || tmpy2 < 0)) {
          ...
          validM2 = new Move(startingSquare, tmpK1,
              setScore(((tmpK1.getXC() * 75) + 20), ((tmpK1.getYC() * 75) + 20), x, y));
      ...
      for (int l = 1; l < 8; l++) {
        ...
        if (!(tmpL2 > 7 || tmpL2 < 0 || tmpy3 > 7 || tmpy3 < 0)) {
          ...
          validM3 = new Move(startingSquare, tmpLMov2,
              setScore(((tmpLMov2.getXC() * 75) + 20), ((tmpLMov2.getYC() * 75) + 20), x, y));
      ...
      for (int n = 1; n < 8; n++) {
        ...
        if (!(tmpN2 > 7 || tmpN2 < 0 || tmpy4 > 7 || tmpy4 < 0)) {
          ...
          validM4 = new Move(startingSquare, tmpNmov2,
              setScore(((tmpNmov2.getXC() * 75) + 20), ((tmpNmov2.getYC() * 75) + 20), x, y));
   ...
   * possibly the simplest piece
   * to get possible movements from. The Knight can essentially move in an L
   * direction from any square on the
   * board as long as the landing square is on the board and we can take an
   * opponents piece but not our own piece.
   */
  ...
  public int setScore(int x, int y, int lastX, int lastY) {
    ...
    else if (!checkBlackOponent(lastX, lastY)) {
    ...
    try {
      ...
      } else if (tmp1.contains("Bishop") || tmp1.contains("Knight")) {

        score = 3;

      } else if (tmp1.contains("Rook")) {

        score = 5;

      } else if (tmp1.contains("Queen")) {
  ...
  private void makeAIMove() {
    ...
    Stack completeMoves = new Stack();
    Move tmp;
    Stack completeMovesB = new Stack();
    Move tmpB;
    Stack black = findBlackPieces();

    while (!black.empty()) {

      Square sB = (Square) black.pop();
      String tmpStringB = sB.getName();
      Stack tmpMovesB = new Stack();
      Stack temporaryB = new Stack();
      /*
       * We need to identify all the possible moves that can be made by the AI
       * Opponent
       */
      if (tmpStringB.contains("Knight")) {
        tmpMovesB = getKnightMoves(sB.getXC(), sB.getYC(), sB.getName());
      } else if (tmpStringB.contains("Bishop")) {
        tmpMovesB = getBishopMoves(sB.getXC(), sB.getYC(), sB.getName());
      } else if (tmpStringB.contains("Pawn")) {
        tmpMovesB = getWhitePawnSquares(sB.getXC(), sB.getYC(), sB.getName());
      } else if (tmpStringB.contains("Rook")) {
        tmpMovesB = getRookMoves(sB.getXC(), sB.getYC(), sB.getName());
      } else if (tmpStringB.contains("Queen")) {
        tmpMovesB = getQueenMoves(sB.getXC(), sB.getYC(), sB.getName());
      } else if (tmpStringB.contains("King")) {
        tmpMovesB = getKingSquares(sB.getXC(), sB.getYC(), sB.getName());
      }

      while (!tmpMovesB.empty()) {
        tmpB = (Move) tmpMovesB.pop();
        completeMovesB.push(tmpB);
      }
      temporaryB = (Stack) completeMovesB.clone();
      getLandingSquares(temporaryB);
      printStack(temporaryB);
    ...
    if (completeMoves.size() == 0) {
       ...
       * need to call the correct agent to select
       * one of these moves. Lets print out the possible moves to the standard output
       * to view what the options are for
       * White. Later when you are finished the continuous assessment you don't need
       * to have such information being printed
       * out to the standard output.
       */
      System.out.println("=============================================================");
      Stack testing = new Stack();
      Stack testBlack = new Stack();
      while (!completeMovesB.empty()) {
        white2Move = false;
        Move tmpMoveB = (Move) completeMovesB.pop();
        Square s1B = (Square) tmpMoveB.getStart();
        Square s2B = (Square) tmpMoveB.getLanding();
        int scoreB = (int) tmpMoveB.getScore();
        System.out.println("The " + s1B.getName() + " can move from (" + s1B.getXC() + ", " + s1B.getYC()
            + ") to the following square: (" + s2B.getXC() + ", " + s2B.getYC() + ")" + s1B.getScore() + scoreB);
        testBlack.push(tmpMoveB);
        white2Move = true;
      }
      while (!completeMoves.empty()) {
        ...
        int score = (int) tmpMove.getScore();
        System.out.println("The " + s1.getName() + " can move from (" + s1.getXC() + ", " + s1.getYC()
            + ") to the following square: (" + s2.getXC() + ", " + s2.getYC() + ")" + s1.getScore() + score);
      ...
      Move selectedMove = agent.SelectedStrategy(strategy, testing, testBlack, white);
  ...
  public void mouseReleased(MouseEvent e) {
    ...
    if (possible) {
      ...
      if (pieceName.contains("Bishop")) {
         ...
         * It moves along the diagonal as long as nothing is in the way to its
         * destination. Like all other Chess
        ...
        if (((landingX < 0) || (landingX > 7)) || ((landingY < 0) || (landingY > 7))) {
           ...
           * same as the y
           * movement, if so we are moving along a diagonal. We then need to identify
           * which diagonal and
         ...
         * as long as there
         * is nothing in its way. It can take an opponents piece and as you know it
         * cannot take its own piece.
         */
        Boolean intheway = false;
        if (((landingX < 0) || (landingX > 7)) || ((landingY < 0) || (landingY > 7))) {
          validMove = false;
          ...
          if (((xMovement != 0) && (yMovement == 0)) || ((xMovement == 0) && (yMovement != 0))) {
            ...
            if (intheway) {
               ...
               * make try and
               * move the piece. The other condiion that we need to check is to see if there
               * is a piece at
         ...
         * xMovement == 1 then the yMovement
         * must equal 2 and also the other way around...
         * We need to check the square that we are moving to and make sure that if there
         * is a piece present
        ...
        if ((xMovement == 0) && (yMovement == 0)) {
          ...
          if (((landingX < 0) || (landingX > 7)) || ((landingY < 0) || (landingY > 7))) {
            ...
            if ((xMovement > 1) || (yMovement > 1)) {
               ...
               * there is at least one square between
              ...
              if ((getPieceName((e.getX() + 75), e.getY()).contains("King"))
                 ...
                 * that the King cannot takes its
        ...
        if (((landingX < 0) || (landingX > 7)) || ((landingY < 0) || (landingY > 7))) {
          ...
          if (xMovement == yMovement) {
            ...
            if (inTheWay) {
               ...
               * two cases:
               * - there is a piece on the square we are trying to move to and we need to make
               * sure that its
               * an enemy piece
            ...
            if (inTheWay) {
               ...
               * Queen should be able to make the
               * movement. We need to check if there is something on the square that we are
               * moving to and if so make sure its
               * an enemy piece and if there is nothing on the square that we are moving to
               * ... make the move.
               */
              if (piecePresent(e.getX(), (e.getY()))) {
                if (pieceName.contains("White")) {
